...
var movie1 = require('./dbConn');
console.log(movie1);
var conn = movie1.connect();


var baseUrl = 'https://movie.douban.com/j/search_subjects?type=movie&tag=%E5%96%9C%E5%89%A7&sort=recommend&page_limit=20&page_start=0';  //列表链接
var arrNew=[]; 
var arrUrl = [];
var errLength=[]; 
            ...
            var fetchUrl = function (myurl, callback) {
                ...
                .end(function (err, ssres) {
                    ...
                    getDetail($,function(obj){
                        ...
                        res.write('<br/>');
                        res.write('id-->  '+movie_id);
                        res.write('<br/>');
                        res.write('简介-->  '+obj.desc);
                        res.write('<br/>');
                        res.write('<br/>');
                        ...
                        var tableSql2 = 'insert into tb_movie_detail(movie_id, director, actors, describle, up_time,rank,count) value(?,?,?,?,?,?,?)';
                        var data2 = [movie_id, obj.director, obj.actors, obj.desc, obj.time,obj.rank,obj.count];        
...
function getMovieList(sres){
    ...
    for(var i = 0;i<arr.length; i++){
        if(arrNew.indexOf(arr[i].id) == -1){
            ...
            var tableSql1 = 'insert into tb_movie(movie_id,movie_name,pic,grade) value(?,?,?,?)';
...
function getDetail($,callback){
    ...
    var rank = Math.floor(Math.random()*20);
    var count = Math.random()*10000000000;

            ...
            var fetchUrl = function (myurl, callback) {
                ...
                .end(function (err, ssres) {
                    ...
                    getDetail($,function(obj){
                        ...
                        res.write('<br/>');
                        var tableSql2 = 'insert into tb_movie_detail(movie_id, director, actors, describle, up_time) value(?,?,?,?,?)';
                        var data2 = [movie_id, obj.director, obj.actors, obj.describle, obj.time];        

                        movie1.insertFun(conn, tableSql2, data2, function(){});
                        //movie1.insertFun(conn, movie_id, obj.director,obj.actors,obj.desc,obj.time, function(){});
...
function getMovieList(sres){
    ...
    for(var i = 0;i<arr.length; i++){
        if(arrNew.indexOf(arr[i].id) == -1){
            ...
            //console.log(data1[1]);
            movie1.insertFun(conn,tableSql1, data1, function(){});
...
function getDetail($,callback){
    ...
    var describle = $('#link-report span').text();
